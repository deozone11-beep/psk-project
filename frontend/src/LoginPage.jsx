...
export default function LoginPage() {
  ...
  const [busyText, setBusyText] = useState('Checking...');
  ...
  async function submit(e) {
    ...
    const slowTimer = setTimeout(() => {
      setBusyText('Waking up server (Render cold-start)...');
    }, 3500);
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        setError('Server took too long to respond. Render may still be booting up. Please try again.');
      } else if (err.message === 'Failed to fetch') {
        setError('Cannot connect to server. Render service might be restarting.');
      } else {
            ...
            <form onSubmit={submit}>
              {activeTab === 'enquiry' ? (
                ...
                <>
                  ...
                  <button className="primary" disabled={busy}>
                    {busy ? busyText : 'Login'} <ArrowRight size={16} />
                  </button>
                </>
              )}
            </form>
