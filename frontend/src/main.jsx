...
async function submit(e){
  ...
  setAppLoading(true);
  try{
    let r=await fetch(`${API}/enquiries`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(new FormData(form)))});
    if(!r.ok){
      const errBody=await r.json().catch(()=>null);
      console.error('Enquiry failed:',r.status,errBody);
      setMsg(errBody?.message||`Error ${r.status}`);
      setAppLoading(false);
      return;
    }
    setMsg(lang==='ta'?'மிக்க நன்றி! விரைவில் தொடர்பு கொள்வோம்.':'Thank you! We will contact you shortly.');
    form.reset();
    setStep(1);
    setTimeout(()=>{setShowEnquiryModal(false);setMsg('')},2500);
  }catch(err){
